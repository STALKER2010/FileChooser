...
public abstract class FMActivity extends ListActivity
{
	...
	public void inflateList(File[] fs)
	{
		...
		SimpleAdapter adapter = new SimpleAdapter(FMActivity.this, listItems,
												  R.layout.list_item,
	...
	protected Dialog onCreateDialog(int id)
	{
		if (id == LONGPRESS && opts.allowFSModifying)
		{
			...
			return new AlertDialog.Builder(FMActivity.this).setItems(new String[] {"Rename","Delete"}, new DialogInterface.OnClickListener() {
		...
		if (paramInt == KeyEvent.KEYCODE_BACK)
			try
			{
				if (!path.getCanonicalPath().equals("/"))
				{
					...
					cur = path.listFiles();
					inflateList(cur);
					if (opts.changeActionBar) ab.setSubtitle(path.getAbsolutePath());
				}
				else
				{
